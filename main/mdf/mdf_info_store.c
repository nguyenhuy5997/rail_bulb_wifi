...
//    MDF_ERROR_CHECK(ret != ESP_OK, ret, "Open non-volatile storage");
    if(ret != ESP_OK) {
    	ESP_LOGW("NVS", "Open non-volatile storage fail");
    }
...
//        MDF_LOGI("<ESP_ERR_NVS_NOT_FOUND> Get value for given key, key: %s", key);
        ESP_LOGW("NVS", "Non-volatile ESP_ERR_NVS_NOT_FOUND");
        return ESP_ERR_NVS_NOT_FOUND;
    }

//    MDF_ERROR_CHECK(ret != ESP_OK, ret, "Get value for given key, key: %s", key);
    if(ret != ESP_OK) {
    	ESP_LOGW("NVS", "Get value for given key, key %s fail", key);
    }
    else
    {
    	ESP_LOGW("NVS", "Get value for given key, key %s ok", key);
    }
    return ESP_OK;
}
